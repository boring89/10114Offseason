...
public class SwerveModule {
    ...
    // SwerveModule(駕駛馬達編號、轉向馬達編號、駕駛馬達是否反轉、轉向馬達是否反轉、絕對編碼器編號、絕對編碼器偏移量(在Phoenix裡zeroHeading後可設為0)、絕對編碼器是否反轉)
    // 此為單一全向輪模組之程式，被SwerveSubsystem調用，詳細數值存於Costants之中的ModuleConstants
    ...
    public SwerveModule(int driveMotorId, int turningMotorId, boolean driveMotorReverdsed, boolean turningMotorReversed, 
        ...
        //
            ...
            turningMotor.configure(FlexConf, SparkBase.ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
            ...
            // 目前的全向輪狀態 SwerveModulePosition(double 駕駛馬達數值, Rotation2d 轉向馬達數值)
    ...
    public SwerveModuleState getState() {
        return new SwerveModuleState(getDriveVelocity(),
            Rotation2d.fromRadians(getAbsoluteEncoderRad()));       // 取得當前全向輪狀態
    }
